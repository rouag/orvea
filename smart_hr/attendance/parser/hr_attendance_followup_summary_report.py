...
class hr_attendance_followup_summary_report_parser(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context):
        super(hr_attendance_followup_summary_report_parser, self).__init__(cr, uid, name, context=context)
    ...
    def _get_employees(self, employee_ids):
    ...
    def _get_employee_ids(self, employee_ids, all_employees):
        ...
        if all_employees:
            emp_ids = employee_obj.search(self.cr, self.uid, [])
            ...
            for emp in employee_obj.browse(self.cr, self.uid, employee_ids):
                if emp.schedule_ids:
                    att_ids = att_obj.search(self.cr, self.uid, [
                        ('employee_id', '=', emp.id),
                    ])
    ...
    def _get_followup(self, employee_ids, all_employees, date_from, date_to):
        # Objects
        ...
        for employee_id in self._get_employee_ids(employee_ids, all_employees):
            ...
            att_rec = {
                'id': seq,
                'employee_id': emp_obj.browse(self.cr, self.uid, employee_id).display_name,
...
class hr_attendance_followup_summary_report(models.AbstractModel):
    _name = 'report.smart_hr.hr_attendance_followup_summary_report'
    _inherit = 'report.abstract_report'
    _template = 'smart_hr.hr_attendance_followup_summary_report'
    _wrapped_report_class = hr_attendance_followup_summary_report_parser