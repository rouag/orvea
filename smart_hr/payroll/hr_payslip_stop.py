...
class HrPayslipStop(models.Model):
    ...
    order_date = fields.Date(string='تاريخ القرار', required=1, default=fields.Datetime.now(), readonly=1,
    ...
    period_ids = fields.One2many('hr.payslip.stop.line', 'payslip_id', string='الفترات', readonly=1,
                                 states={'draft': [('readonly', 0)]})
...
class HrPayslipStopRun(models.Model):
    ...
    period_ids = fields.Many2many('hr.period', string='الفترات', readonly=1, states={'draft': [('readonly', 0)]})
    ...
    def button_refuse(self):
        for rec in self:
            for employee in rec.employee_ids:
                payslip_stop_obj = self.env['hr.payslip.stop']
                payslip_stop = payslip_stop_obj.search(
                    [('employee_id', '=', employee.id), ('name', '=', rec.name), ('order_date', '=', rec.order_date)])
                if payslip_stop:
                    for line in payslip_stop.period_ids:
                        line.write({'stop_period': False, 'period_id': line.period_id.id, 'state': 'refused'})
