...
class HrOvertime(models.Model):
    ...
    line_ids = fields.One2many('hr.overtime.ligne', 'overtime_id', string=u'خارج دوام', readonly=1,
...
class HrOvertimeLigne(models.Model):
    ...
    employee_id = fields.Many2one('hr.employee', string=u' الموظف', required=1,
                                  Domain=[('emp_state', '!=', 'suspended'), ('emp_state', '!=', 'terminated')])
    ...
    type_compensation = fields.Selection([('compensatory_leave', ' إجازة تعويضية'),
                                          ('amount', 'مقابل مادي')], string='نوع التعويض', required=1,
                                         default='compensatory_leave')
