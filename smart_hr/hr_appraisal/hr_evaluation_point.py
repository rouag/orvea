...
class hr_evaluation_point(models.Model):
    ...
    functionality_ids = fields.One2many('hr.evaluation.point.functionality', 'hr_evaluation_point_id', string=u'نقاط  الإداء الوظيفي')
...
class hr_evaluation_point_seniority(models.Model):
...
class hr_evaluation_point_training(models.Model):
    ...
    type_training = fields.Selection([('direct_experience', 'الخبرات المباشرة'), ('indirect_experience', 'الخبرات غير المباشرة')], string='نوع التدريب',required=1 )
...
class hr_evaluation_point_functionality(models.Model):
...
class hr_evaluationResultfoctionality(models.Model):
    ...
    point_from = fields.Float(string=u"من ",required=1)
    point_to = fields.Float(string=u"إلى ",required=1)
