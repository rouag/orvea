...
class HrAppraisal(models.Model):
    ...
    mail_template_id = fields.Many2one('mail.template', string="Email Template for Appraisal",
                                       default=lambda self: self.env.ref('smart_hr.send_appraisal_templates'))

    @api.multi
    def _compute_note_final(self):
        completed_survey = self.env['survey.user_input'].read_group(
            [('appraisal_id', 'in', self.ids), ('state', '=', 'done')], ['appraisal_id'], ['appraisal_id'])
