...
class HrHolidays(models.Model):
    ...
    extended_leave_id = fields.Many2one('hr.holidays', string=u'الإجازة الممددة')
    parent_id = fields.Many2one('hr.holidays', string=u'Parent')
    extension_leave_ids = fields.One2many('hr.holidays', 'parent_id', string=u'التمديدات')
    # Cancellation
    ...
    leave_cancellation_ids = fields.One2many('hr.holidays.cancellation', 'leave_id', string=u'طلبات الإلغاء')
    # Sick Leave Fields
...
#             extended_leave = self.env['hr.holidays'].browse(extended_leave_id)
...
#         # Objects
#         leave_cancellation_obj = self.env['hr.holidays.cancellation']
#         # Variables
#         user = self.env['res.users'].browse(self._uid)
