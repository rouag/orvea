...
class SmartUtils(models.Model):
    ...
    def get_overlapped_periode(self, date_from, date_to, operation_date_from, operation_date_to):
        ...
        """
        operation_date_from = fields.Date.from_string(str(operation_date_from))
        operation_date_to = fields.Date.from_string(str(operation_date_to))
        date_from = fields.Date.from_string(str(date_from))
        date_to = fields.Date.from_string(str(date_to))
        
        if operation_date_from == date_from and operation_date_to == date_to:
            return operation_date_from, operation_date_to
        elif operation_date_from > date_from and operation_date_to < date_to:
            return operation_date_from, operation_date_to
        elif operation_date_from < date_from and operation_date_to == date_from:
            return operation_date_to, operation_date_to
        elif operation_date_from == date_to and operation_date_to > date_to:
            return operation_date_from, operation_date_from
        elif operation_date_from < date_from and operation_date_to > date_from:
            return date_from, operation_date_to
        elif operation_date_from < date_to and operation_date_to > date_to:
            return operation_date_from, date_to
        elif operation_date_from < date_from and operation_date_to > date_to:
