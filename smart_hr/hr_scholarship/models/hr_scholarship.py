...
class HrScholarship(models.Model):
    ...
    employee_id = fields.Many2one('hr.employee', string=u'الموظف', required=1,
                                   default=lambda self: self.env['hr.employee'].search([('user_id', '=', self._uid)], limit=1), advanced_search=True)
    ...
    diplom_type = fields.Selection([('hight', u'دبلوم من الدراسات العليا'), ('middle', u'الليسانس او الباكالوريوس او دبلوم متوسط')], string='نوع الدبلوم', required=1)
    ...
    Faculty_id = fields.Many2one('res.partner', string="  الجامعة/المعهد", required=1, domain=[('company_type', 'in',['faculty','school'])])
    ...
    file_decision = fields.Binary(string=u'الخطاب')
    ...
    def action_done(self):
        ...
        self.env['hr.employee.history'].sudo().add_action_line(self.employee_id, self.num_speech, self.date_speech, type)
        self.state = 'done'
...
#         constraint service_duration and education level
        ...
        if self.diplom_type == 'hight':
            ...
            for level in education_level_ids:
                if (date_from - fields.Date.from_string(level.diploma_date)).days < 2 * 365:
                    raise ValidationError(u"لم تمضي سنتين   في الخدمة بعد الحصول على درجة الباكالوريوس او الليسانس")
        ...
        if self.diplom_type == 'middle':
            ...
            for level in education_level_ids:
                if (date_from - fields.Date.from_string(level.diploma_date)).days < 3 * 365:
                    raise ValidationError(u"لم تمضي ثلاث سنوات في الخدمة  بعد الحصول على دبلوم الثانوية")
            if not education_level_ids:
                raise ValidationError(u"الرجاء مراجعة المؤهلات العلمية للموظف")
...
#         constraint evaluation
        ...
        employee_evaluation_id2 = self.env['hr.employee.evaluation.level'].search([('employee_id', '=', self.employee_id.id), ('year', '=', date_from.year - 2)], limit=1)
        if employee_evaluation_id1 and employee_evaluation_id2:
            if employee_evaluation_id1.degree_id.point_to < self.env.ref('smart_hr.assessment_hr_good').point_from or employee_evaluation_id1.degree_id.point_to < self.env.ref('smart_hr.assessment_hr_good').point_from:
